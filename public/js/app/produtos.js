$(document).ready(function () {
  ...
  const modalProdutos = $("#modal_products");
  ...
  const CardPDF = (device, productId, file, linkAlt, isLiberado, csrfName, csrfValue) => {
    if (isLiberado) {
      return `
        <form method="POST" id="product-${productId}" data-liberado="${isLiberado}" action="leitor">
  ...
  const renderizar = (device, codigo_item, item, isLiberado) => {
    let html = '';
    if (item.tipo == 'video') {
      html = CardVideo(device, codigo_item, item.nome, item.linkAlt, isLiberado);
    } else if (item.tipo == 'link') {
      html = CardLink(device, codigo_item, item.link, item.linkAlt, isLiberado);
    } else {
      html = CardPDF(device, codigo_item, item.file, item.linkAlt, isLiberado, params[1], params[2]);
    }
    divProducts.append(html);
  }

  const montar_produtos = (produtos_liberados) => {
    ...
    for (codigo_item in itens) {
      ...
      if (item.original != undefined) {
        codigo_item = item.original;
        item = itens[item.original];
      }

      if (codigo_item in itens_renderizados) {
        /*
        **item ja renderizado, conferir se o item ja renderizado estava bloqueado (situação automatica)
        **se o item antigo ja esta liberado, não faça nada
        **se o item antigo e atual estão bloqueados, não faça nada
        */
        if (!itens_renderizados[codigo_item] && isLiberado) {
          //item atual esta liberado enquanto o antigo estava bloqueado, remover o antigo
          $(`#product-${codigo_item}`).remove();
          renderizar(device, codigo_item, item, isLiberado);
        }
      } else {
        itens_renderizados[codigo_item] = isLiberado;
        renderizar(device, codigo_item, item, isLiberado);
      }

    }

    //Renderizando extras do 36673
